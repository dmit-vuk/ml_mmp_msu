...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        posteriors = []
        for sentence in parallel_corpus:
            p_a = self.translation_probs[sentence.source_tokens, :][:, sentence.target_tokens]
            p_a = p_a / p_a.sum(axis=0)
            posteriors.append(p_a)
        return posteriors
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            n_r = len(sentence.source_tokens)
            target_probs = self.translation_probs[sentence.source_tokens, :][:, sentence.target_tokens]
            elbo += (np.log(np.maximum(target_probs / (n_r * posterior + 1e-8), 1e-8)) * posterior).sum()
        return elbo
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for sentence, posterior in zip(parallel_corpus, posteriors):
            np.add.at(self.translation_probs, (sentence.source_tokens.reshape(-1, 1), sentence.target_tokens), posterior)
        self.translation_probs /= self.translation_probs.sum(1, keepdims=True)
    ...
    def align(self, sentences):
        ...
        for sentence in sentences:
            target_probs = self.translation_probs[sentence.source_tokens, :][:, sentence.target_tokens]
            alignment = [(i+1, j+1) for i, j in zip(target_probs.argmax(axis=0), range(target_probs.shape[1]))]
            result.append(alignment)
        return result
